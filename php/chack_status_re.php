...
if ($act == 'read_drive') {
    $sql = "SELECT d.*,s.status_id,s.status_name,l.id, l.log_name,m.* FROM `drive_golf` d LEFT JOIN status s ON d.status_id = s.status_id  LEFT JOIN log_golf_driving l ON d.id_log = l.id LEFT JOIN member m ON d.reserve_by = m.Member_id WHERE  d.status_id = '5'";
    // die($sql);
    $result = $conn->query($sql);
    echo json_encode($result->fetch_all(MYSQLI_ASSOC));
}
...
if($act == 'cencel'){
    $sql = "UPDATE `book` SET `status_id`= '12',`operated_by`='$cencel_by',`note`='$cancelReason' WHERE Book_id = '$bookId' ";
    // die($sql);
    if($conn -> query($sql)){
        $msg = "ยกเลิกรายการสำเร็จ";
    }else{
        $msg = "ยกเลิกรายการไม่สำเร็จ";
    }
    echo $msg;
}
if($act == 'update_log_end'){
    $id_log = isset($_POST['id_log']) ? $_POST['id_log'] : "";
    $id_drive = isset($_POST['id_drive']) ? $_POST['id_drive'] : "";

    $sql = "UPDATE `drive_golf` SET `status_id`='16' WHERE `id_drive`='$id_drive'";
    // die($sql);
...
if ($act == "sendEmail") {
    ...
    $sql = "SELECT b.*, s.status_id, s.status_name, g.*, m.* FROM book b LEFT JOIN status s ON b.status_id = s.status_id LEFT JOIN golf_course g ON b.Book_time = g.Course_id LEFT JOIN member m ON b.Member_id = m.Member_id WHERE b.id = '$id'";
    // die($sql);
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        ...
        foreach ($data as $row) {
            if (!sendEmail($row['Member_id'], $row['Member_email'], $row['time'], $row['Book_date'], $row['Member_firstname'], $row['Member_lastname'])) {
...
function sendEmail($user, $email,$time,$Book_date,$Member_firstname,$Member_lastname) {
...
<body>
    <div style="width: 100%;display:flex;justify-content: center;">
        <div style="border: 1px solid #c4c7c5;border-radius: 10px;padding: 10px 20px; width: 50%;">
            <p style="font-size: 20px;width: 100%;text-align:center;">แจ้งเตือนกำหนดการวันออกรอบในวันที่' . $Book_date . '</p>
            <hr style="width: 100%;">
            ...
            <p style="font-size: 25px; width: 100%;text-align: center;">เวลาที่จองออกรอบ : ' . $time . '</p>
            <p style="font-size: 25px; width: 100%;text-align: center;">วันที่จอง : ' . $Book_date . '</p>
