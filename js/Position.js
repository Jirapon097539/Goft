...
function display() {
    $.post(prg, { "act": 'read' }, function(data, status) {
        row = "<div class = 'card'>"
        row += "<div class = 'card-header alert-light text-white'style ='background-image: linear-gradient(rgb(76, 122, 228), rgb(28, 114, 243));'>จัดการข้อมูลตำแหน่ง "
        row += "</div>"
        ...
        $.each(posit, function(index, obj) {
            row += "<tr>"
            row += "<td>" + obj.position_id + "</td>"
            row += "<td>" + obj.position_name + "</td>"
            row += "<td><button type='button' class='btn btn-warning'onclick = edit('" + index + "')>แก้ไข</button> &nbsp;&nbsp;"
            row += "<button type ='button' class ='btn btn-danger' onclick = del('" + obj.position_id + "')>ลบ</button ></td>"
            row += "</tr>"
        });
        row += "</tbody>"
        row += "</table>"
...
function edit(i) {
    ...
    $("#position_name").val(posit[i].position_name);
...
function del(position_id) {
    if (confirm('ต้องการลบ ? ')) {
        $.post(prg, { position_id, act: 'delete' }, function(data, status) {
